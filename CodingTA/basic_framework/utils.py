...
# 这一段代码的主要目的是统一代码风格
# 首先解析成语法树然后再解析回去，这样就消除了注释
# 然后假如除了函数定义行之外还有没缩进的代码，无视掉
def regularize(code):
    ...
    # 神经病把费半天劲造一个 tmp_list 最后用也不用
    # 将他们解析为一个一个的 token(e.g (type=1(NAME), "search", start=(3, 4), end=(3, 10), line="def search(x, seq):\n"))
    #       一行可以解析很多很多 token，变量名、"\n"都分别是一个 token
    new_line_flag = False
    for token in token_list:
        if tok_name[token.exact_type] in ["NEWLINE", "ENDMARKER"]: # "\n"或者 EOF
            ...
            tmp_list = []
            new_line_flag = True
        elif tok_name[token.exact_type] == "NL": # 无意义的字符
            pass
        elif tok_name[token.exact_type] == "COMMENT":
            pass
        elif tok_name[token.exact_type] == "INDENT": # 缩进
            indent_str += "    "
        elif tok_name[token.exact_type] == "DEDENT": # 我也不知道是啥反正也是调整缩进的
            if new_line_flag:
...
# return : List[String] 一段代码中所有的变量的名字
